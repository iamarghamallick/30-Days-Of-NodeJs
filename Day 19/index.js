...
mongoose.connect('mongodb://localhost:27017/argha')
    .then(() => {
...
function addUserWithValidation(user) {
    ...
    // Use the save method to save the user to the 'users' collection in the 'argha' database
    newUser.save()
        .then(() => {
            // Log a success message if the user is saved successfully
            console.log('User successfully added to the "users" collection in the "argha" database!');
        })
        .catch((error) => {
...
// Example usage:
addUserWithValidation({ username: 'argha', email: 'contact.arghamallick@gmail.com' });
